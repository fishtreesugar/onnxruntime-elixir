defmodule OnnxRuntime.TestConcat do
  use ExUnit.Case
  ...
  |> Enum.each(fn {type_str, type_tuple} ->
    test "Concat 1d tensors #{type_str}" do
      t1 =
        Nx.tensor([1, 2, 3, 4], type: unquote(type_tuple))
    ...
    test "Concat 3d tensors #{type_str}" do
      o1 =
        Nx.iota({2, 3, 5}, type: unquote(type_tuple)) |> Nx.backend_transfer(OnnxRuntime.Backend)

      o2 =
        Nx.iota({1, 3, 5}, type: unquote(type_tuple)) |> Nx.backend_transfer(OnnxRuntime.Backend)
    ...
    test "Concat axis #{type_str} 1" do
      o1 = Nx.iota({3, 5}, type: unquote(type_tuple)) |> Nx.backend_transfer(OnnxRuntime.Backend)
      o2 = Nx.iota({3, 5}, type: unquote(type_tuple)) |> Nx.backend_transfer(OnnxRuntime.Backend)
    ...
    test "Concat axis 1 of three 3-dimensional #{type_str} vector" do
      t1 =
        Nx.iota({3, 5, 7}, type: unquote(type_tuple)) |> Nx.backend_transfer(OnnxRuntime.Backend)
    ...
    test "Concat does not alter component #{type_str} vectors" do
      t1 =
        Nx.tensor([1, 2, 3, 4], type: unquote(type_tuple))
        |> Nx.backend_transfer(OnnxRuntime.Backend)
  ...
  test "Concat fails to concat vectors of differing types" do
    assert_raise RuntimeError,
                 "OnnxRuntime does not currently support concatenation of vectors with differing types.",
                 fn ->
                   t1 =
                     Nx.tensor([1, 2, 3], type: {:s, 16})
                     |> Nx.backend_transfer(OnnxRuntime.Backend)
